...
import {
  BOSSES, ARCANA, WEAPONS, EDITIONS,
  BOSS_STATUS_COLORS,
} from '@/data/game-data';
import { Skull, Sparkles, Swords, TrendingUp } from 'lucide-react';
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {moduleCards.map((card) => {
              ...
              return (
                <Link key={card.key} href={card.href} title={card.title} className="group block rounded-2xl bg-white/[0.03] border border-white/[0.08] p-6 hover:border-[var(--color-accent)]/40 hover:shadow-[0_0_30px_rgba(240,176,48,0.1)] transition-all duration-300 backdrop-blur-sm card-accent-bar">
      ...
      {/* ===== BOSS ROSTER TABLE (Data-Driven Table 1) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_boss_table_label')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_boss_table_title')}</h2>
            </div>
            <Link href="/bosses" title={t('nav_bosses')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_boss_table_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_4rem_1.5fr_1.5fr] sm:grid-cols-[2.5rem_1.5fr_4rem_2fr_2fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_boss_name')}</span><span>{t('home_boss_nature')}</span><span>{t('home_boss_style')}</span><span>{t('home_boss_status')}</span>
            </div>
            {BOSSES.map((boss, idx) => {
              const status = t(boss.statusKey);
              const sc = BOSS_STATUS_COLORS[status.toLowerCase()] ?? 'var(--color-tier-c)';
              return (
                <Link key={boss.id} href="/bosses" title={t(boss.nameKey)} className="grid grid-cols-[2rem_1fr_4rem_1.5fr_1.5fr] sm:grid-cols-[2.5rem_1.5fr_4rem_2fr_2fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                  ...
                  <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                    <Skull className="w-4 h-4 text-[var(--color-accent)] shrink-0" />
                    {t(boss.nameKey)}
                  </span>
                  <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ color: sc, background: `${sc}15` }}>{status}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(boss.styleKey)}</span>
                  <span className="text-xs text-[var(--color-text-secondary)]">{t(boss.natureKey)}</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* ===== ARCANA CARDS (Data-Driven Cards 1) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_arcana_table_label')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_arcana_table_title')}</h2>
          </div>
          <Link href="/arcana" title={t('nav_arcana')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_arcana_table_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {ARCANA.map((card) => (
            <Link key={card.id} href="/arcana" title={t(card.nameKey)} className="category-card group block rounded-2xl overflow-hidden">
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles className="w-4 h-4 text-[var(--color-accent)] shrink-0" />
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-[var(--color-tier-s)] bg-[var(--color-tier-s)]/10">{t(card.badgeKey)}</span>
                </div>
                <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(card.nameKey)}</h3>
                <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t('home_arcana_focus')}: </span><span className="meta-value">{t(card.doesKey)}</span></p>
                <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t('home_arcana_best')}: {t(card.whyKey)}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* ===== ARSENAL TABLE (Data-Driven Table 2) ===== */}
      <section className="bg-[var(--color-bg-secondary)] border-y border-[var(--color-border)] full-width-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="section-label">{t('home_weapon_cards_label')}</div>
              <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_weapon_cards_title')}</h2>
            </div>
            <Link href="/weapons" title={t('nav_weapons')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
          </div>
          <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_weapon_cards_desc')}</p>
          <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] overflow-hidden backdrop-blur-sm">
            <div className="grid grid-cols-[2rem_1fr_4rem_2.5fr] sm:grid-cols-[2.5rem_1.5fr_5rem_3fr] gap-x-4 text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider border-b border-[var(--color-border)] px-4 py-3">
              <span></span><span>{t('home_weapon_arsenal')}</span><span>{t('home_weapon_badge')}</span><span>{t('home_weapon_effect')}</span>
            </div>
            {WEAPONS.map((weapon, idx) => (
              <Link key={weapon.id} href="/weapons" title={t(weapon.nameKey)} className="grid grid-cols-[2rem_1fr_4rem_2.5fr] sm:grid-cols-[2.5rem_1.5fr_5rem_3fr] items-center gap-x-4 px-4 py-3 border-b border-white/[0.04] last:border-b-0 hover:bg-[var(--color-accent)]/5 transition-colors group">
                <span className="text-xs font-mono text-[var(--color-text-muted)]">{idx + 1}</span>
                <span className="flex items-center gap-2 font-semibold text-sm group-hover:text-[var(--color-accent)] transition-colors">
                  <Swords className="w-4 h-4 text-[var(--color-accent)] shrink-0" />
                  {t(weapon.nameKey)}
                </span>
                <span className="justify-self-start text-[10px] font-bold px-1.5 py-0.5 rounded-full text-[var(--color-tier-a)] bg-[var(--color-tier-a)]/10">{t(weapon.badgeKey)}</span>
                <span className="text-xs text-[var(--color-text-secondary)]">{t(weapon.detailKey)}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* ===== EDITION & PLATFORM CARDS (Data-Driven Cards 2) ===== */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="section-label">{t('home_module_editions')}</div>
            <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-heading)] gradient-text">{t('home_relic_cards_title')}</h2>
          </div>
          <Link href="/editions-preorder" title={t('nav_editionsPreorder')} className="text-sm font-semibold text-[var(--color-accent)] hover:underline flex items-center gap-1">{t('view_all')} <ArrowRight className="w-3 h-3" /></Link>
        </div>
        <p className="text-[var(--color-text-secondary)] mb-6 leading-relaxed">{t('home_relic_cards_desc')}</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {EDITIONS.map((ed) => (
            <Link key={ed.id} href="/editions-preorder" title={t(ed.nameKey)} className="category-card group block rounded-2xl">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-[var(--color-accent)] bg-[var(--color-accent)]/10">{t(ed.badgeKey)}</span>
                <TrendingUp className="w-4 h-4 text-[var(--color-accent)]" />
              </div>
              <h3 className="text-[0.9375rem] font-bold font-[var(--font-heading)] text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors mb-1">{t(ed.nameKey)}</h3>
              <p className="text-[0.75rem] text-[var(--color-text-muted)] mb-2">{t(ed.subtitleKey)}</p>
              <p className="card-meta text-[0.75rem] text-[var(--color-text-secondary)] mb-2"><span className="meta-label">{t(ed.f0LabelKey)}: </span><span className="meta-value">{t(ed.f0ValueKey)}</span></p>
              <p className="card-desc text-[0.8125rem] text-[var(--color-text-secondary)]">{t(ed.f1LabelKey)}: {t(ed.f1ValueKey)}</p>
            </Link>
          ))}
        </div>
      </section>
